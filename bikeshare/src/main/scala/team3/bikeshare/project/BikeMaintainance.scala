...
object BigBikeGraphBikeMaintainance {
  def main(args: Array[String]) {
    ...
    val df = sparkSession.read.option("header","true").csv("src/main/resources/201801_fordgobike_tripdata.csv").cache()
    df.printSchema()
    df.show()
    // println("="*70)
    ...
    df.groupBy("bike_id").agg(count("bike_id")).withColumnRenamed("count(bike_id)", "Number of Trips").sort(desc("Number of Trips")).show(30)
    sparkSession.stop()
  }
}
